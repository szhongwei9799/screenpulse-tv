...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        initWebView()
        initExoPlayer()
        bgMusicPlayer = BackgroundMusicPlayer(this).also {
            WebServer.setBgMusicPlayer(it)
        }
        startWebServer()
        observePlaylist()
    ...
    private fun applyConfig(config: PlaylistConfig) {
        ...
        playlistManager?.setOrderMode(config.orderMode)
        playlistManager?.setRepeatMode(config.repeatMode, config.repeatCount)
        ...
        // Background music
        val bgmEnabled = config.bgMusicEnabled
        if (bgmEnabled) {
            exoPlayer?.volume = 0f
            lifecycleScope.launch(Dispatchers.IO) {
                val db = com.screenpulse.player.data.AppDatabase.getInstance(this@MainActivity)
                val musicList = db.backgroundMusicDao().getAllMusic()
                withContext(Dispatchers.Main) {
                    bgMusicPlayer?.setMusicFiles(musicList.map { it.filePath })
                    bgMusicPlayer?.setVolume(config.bgMusicVolume)
                    bgMusicPlayer?.setLooping(config.bgMusicLoop)
                    bgMusicPlayer?.enable()
                }
            }
        } else {
            bgMusicPlayer?.disable()
    ...
    private fun playVideoItem(item: com.screenpulse.player.data.entity.MediaItem) {
        ...
        // If bg music is enabled, mute video
        if (bgMusicPlayer?.isEnabled() == true) {
            exoPlayer?.volume = 0f
        }
    }
    ...
    override fun onDestroy() {
        ...
        bgMusicPlayer = null
        WebServer.setBgMusicPlayer(null)
        exoPlayer?.release()
        exoPlayer = null
        webServer?.stop()
        webServer = null
